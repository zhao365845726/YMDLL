...
namespace ML.Utilities
{
    ...
    public class CommonHelper
    {
        ...
        /// <returns></returns>
        public T FilterModelData<T>(T ModelClass)
        {
            //SortedDictionary<string, string> sPara = new SortedDictionary<string, string>();
            Type t = ModelClass.GetType();
            PropertyInfo[] resultPro = ModelClass.GetType().GetProperties();
            int i = 0;
            foreach (PropertyInfo pi in t.GetProperties())
            {
                if (pi.GetValue(ModelClass, null) == null || pi.GetValue(ModelClass, null).ToString() == "&nbsp;")
                {
                    resultPro[i].SetValue(ModelClass, "", null);
                    i++;
                    continue;
                }
                object value = pi.GetValue(ModelClass, null);
                resultPro[i].SetValue(ModelClass, value, null);
                i++;
            }
            return ModelClass;
        }
