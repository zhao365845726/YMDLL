...
namespace YMDLL.Class
{
  public class YM_MySQL : IDBProperty
  {
    ...
    /// <param name="sWhere"></param>
    /// <returns></returns>
    public DataTable GetPager(string sTableName, string sColumns, string sWhere, string sGroup, string sOrder, int sPageSize, int sPageIndex,  out int totalCount)
    {
      sWhere = " WHERE 1=1 " + sWhere;
      string strStart = Convert.ToString(sPageIndex * sPageSize);
      DataTable table = new DataTable();
      m_sql = "SELECT " + sColumns + " FROM " + sTableName + sWhere;
      if (!string.IsNullOrEmpty(sGroup))
      {
        m_sql += " GROUP BY " + sGroup;
      }
      if (!string.IsNullOrEmpty(sOrder))
      {
        m_sql += " ORDER BY " + sOrder;
      }
      m_sql += " LIMIT " + strStart + "," + sPageSize.ToString();
      if (this.OpenConnection() == true)
      {
        MySqlDataAdapter adapter = new MySqlDataAdapter(m_sql, connection);
        DataSet ds = new DataSet();
